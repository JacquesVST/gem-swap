...
import { DefaultDialogOption, Dialog, DialogController } from "./Dialog";
...
export class Run {
    ...
    findNextEnemy(): Enemy {
        let stage: EnemyStage = this.findStage();
        if (stage.currentEnemyIndex < this.map.enemyCount - 1) {
    ...
    updateBottomProgressBars(): void {
        let newBars: ProgressBar[] = [
            ProgressBar.yourHealthBar(this.character.health, this.character.currentHealth),
            ProgressBar.yourMovesBar(this.maxMoves, this.character.moves),
        ];

        newBars.forEach((progressBar: ProgressBar, index: number) => {
            this.updateProgressBar(index + 4, progressBar);
        });
    }
    ...
    updateScore(match: Item[]): number {
        ...
        if (this.character.hasReward('Hit Streak')) {
        ...
        this.updateProgressBar(3, ProgressBar.enemyHealthBar(enemy, enemy.currentHealth - finalDamage, this.map.enemyCount), () => {
            enemy.damage(finalDamage, undefined,
                () => {
                    ...
                    if (this.findNextEnemy() instanceof BossEnemy) {
        ...
        this.map.nextEnemy(this, () => {
            ...
            if (!this.map.winState) {
                ...
                this.newShopDialog(() => {
                    this.updateBottomProgressBars();
                    this.updatePlayerStatsAndRewards()
                    this.grid.applyCriticalInBoard();
                    this.sounds['item'].play();
                }, () => {
    ...
    updatePlayerStatsAndRewards(): void {
        ...
        if (this.character?.rewards || this.character?.activeItem) {
            ...
            rewardsToShow.forEach((reward: Reward, index: number) => {
                ...
                if (reward.price || reward.unique) {

                    rewardsContent += reward.price ? `<span class="price">$ ${reward.price}</span><br>` : '';
                    rewardsContent += reward.unique ? `<span class="unique">Unique</span><br>` : '';
    ...
    newRandomDropDialog(callback: () => void) {
        ...
        let dialog: Dialog = new Dialog(
            ...
            'You may take it',
            this.dialogController.rewardListToDialogOption(rewardList, this, callback),
            new Color(40, 40, 40),
            false,
            true
        );
    ...
    newPercDialog(callback: () => void) {
        let rewardList: Reward[] = this.generateRewardsBasedOnRarity(
            this.rewardOptions,
            RewardPools.defaultPool(this),
            ['Common', 'Rare', 'Epic']
        );
    ...
    newShopDialog(selectCallback: () => void, closeCallback?: () => void) {
        ...
        rewardList = [RewardPools.fullHealthShopItem(this), ...rewardList]
        ...
        let dialog: Dialog = new Dialog(
            ...
            this.dialogController.rewardListToDialogOption(rewardList, this, selectCallback),
            new Color(40, 40, 40),
            true,
            false,
            closeCallback
    ...
    newInitialRewardDialog(callback?: () => void) {
        let rewardList: Reward[] = this.generateRewardsBasedOnRarity(3, RewardPools.defaultPool(this), ['Common']);
    ...
    generateRewardsBasedOnRarity(amount: number, pool: Reward[], rarities: string[]): Reward[] {
        ...
        rewardsOfRarity.forEach((rewards: Reward[]) => {
            rewards = rewards.filter((reward: Reward) => !(this.character.hasReward(reward.name) && reward.unique));
            let initialLength: number = rewardList.length;
            do {
                let random: number = Math.floor(Math.random() * rewards.length);

                if (!rewardList.map((reward: Reward) => reward.name).includes(rewards[random].name)) {
